...
async def hackrx_run(request: RunRequest, _auth=Depends(verify_bearer_token)):
    ...
    async def ask_question(batch):
        ...
        optimized_query = (f"Pls give a precise answer in a single line for each question.each question is seprated by '_' pls seprate each anser by '_' as well. {joined}")
